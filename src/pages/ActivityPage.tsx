            ...
            <div className="table-wrap">
              <table className="table">
                ...
                <tbody>
                  {pageRows.map((log) => (
                    <tr key={log.id}>
                      <td className="whitespace-nowrap text-ink-soft dark:text-cream-300">{formatDateTime(log.created_at)}</td>
                      <td className="font-semibold text-ink dark:text-cream-100">{log.user?.full_name || log.user?.email || '—'}</td>
                      <td><Badge tone="brown">{log.action}</Badge></td>
                      <td className="text-ink-soft dark:text-cream-300">{log.entity_type || '—'}</td>
                      <td className="max-w-[120px] truncate font-mono text-xs text-ink-faint dark:text-cream-400/70">{log.entity_id ?? '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
